...
class FightSimulator:
    ...
    def simulate_frame(self, frame: int, game_state_manager: GameStateManager) -> None:
        # 1. Process general frame state from rules
        # TODO: Will separate once attack damage animations come into play
        for player_id in game_state_manager.game_state.player_states:
            ...
            if player_state.input_buffer:
                if player_state.input_buffer.is_empty():
                    ...
                    if not self.attack_manager.has_attack(player=player_state.id):
